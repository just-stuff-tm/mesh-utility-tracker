...
- **2026-02-06**: Changed node discovery to `CMD_SEND_TELEMETRY_REQ` (0x27) with broadcast address 0xFFFF — collects `StatusResponse` (0x87) push events during 20s window with RSSI/SNR stats, then `getContacts()` to match pubKeyPrefixes to contact names. No separate getStatus() calls needed.