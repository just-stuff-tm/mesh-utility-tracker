...
- **2026-02-07**: Updated signal quality scoring to combined RSSI+SNR with LoRa-appropriate thresholds: Excellent (RSSI>-90, SNR>10), Good (>-100, >0), Fair (>-110, >-7), Marginal (>-115, >-13), Poor (>-120), No Link (≤-120), Noisy (RSSI>-115 but SNR≤-13, shown in purple). Quality = min(RSSI level, SNR level). Based on MeshCore community LoRa link reference.
...
- **2026-02-07**: Added offline-first PWA support — Service worker (sw.js) caches app shell, map tiles (cache-first), CDN assets, and API responses. IndexedDB (Dexie.js) stores nodes, scan results, coverage zones, and observers locally. Queries fall back to IndexedDB when offline. POST mutations queue in outbox when offline, drain automatically on reconnect. Map tiles cached as browsed with optional area download in settings. Offline status indicator in header. Sync controls in settings panel.
- **2026-02-07**: Added tile caching toggle — Switch in settings (data-testid="switch-tile-caching") enables/disables automatic map tile caching by service worker. Persisted via localStorage key "mesh-tile-caching". Communicated to SW via SET_TILE_CACHING message. When off, previously cached tiles still served but new ones aren't saved. Clear Tile Cache button removes all cached tiles via CLEAR_TILE_CACHE SW message.
- **2026-02-07**: Added "Update Radio Position" toggle — OFF by default. When off, scans do NOT call setAdvertLatLong() on the radio, preventing the webapp from overwriting the Companion's stored position coordinates. When on, GPS coordinates are sent to the radio during scans. Persisted via localStorage key "mesh_update_radio_position".
- **2026-02-07**: Fixed dead zone clearing — after a successful scan result, any dead zone records at the same hex cell (within 0.0001° tolerance) are deleted from the database, so zones that receive signal are no longer stuck as dead zones. Uses `clearDeadZonesNear()` in storage.
- **2026-02-07**: Fixed Leaflet popup z-index — `.leaflet-popup` and `.leaflet-popup-pane` set to z-index 1200 to ensure popups appear above map controls and layer switcher on mobile.