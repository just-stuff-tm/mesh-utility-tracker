...
- **2026-02-06**: Node discovery uses `SendStatusReq` (cmd 27) with broadcast 0xFFFF — collects `StatusResponse` (0x87) push events with RSSI/SNR stats + `NewAdvert` (0x8A) push events with contact info during 20s window. Matched by pubKeyPrefix. Fully event-driven, no getContacts() or getStatus() calls needed.