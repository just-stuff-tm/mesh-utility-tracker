...
export default function SettingsPage() {
  ...
  const allZones = useMemo(() => aggregateScansToZones(rawScans), [rawScans]);
  const deadZones = useMemo(() => allZones.filter((z) => z.isDeadZone), [allZones]);

  // Note: Dead zone removal is now read-only since there's no backend
  // In a full implementation, you'd need to implement zone exclusion in localStorage
          ...
          <div className="space-y-2">
            {deadZones.map((zone) => (
              ...
              >
                ...
                {/* Dead zones are computed from scans - no backend to delete from */}
              </div>
            ))}
          </div>
        )}
